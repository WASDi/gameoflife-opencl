...
package org.wasd.jocl.shit.old;
 ...
 */
public class MagicDots {

    public static final int SIZE = 640;
    public static final int NUM_DOTS = 50;
    ...
    public static void main(String args[]) {
        SwingUtilities.invokeLater(MagicDots::new);
    }
    ...
    public MagicDots() {
        ...
        fancyImage = createBufferedImage(fileName);

        outputImage = new BufferedImage(
                fancyImage.getWidth(), fancyImage.getHeight(), BufferedImage.TYPE_INT_RGB);

        // Create the panel showing the input and output images
        JPanel mainPanel = new JPanel(new GridLayout(1, 0));
        JLabel inputLabel = new JLabel(new ImageIcon(fancyImage));
    ...
    void initCL() {
        ...
        cl_program program = clCreateProgramWithSource(context,
                1, new String[]{KernelFile.MAGIC_DOTS.load()}, null, null);
        ...
        kernel = clCreateKernel(program, KernelFile.MAGIC_DOTS.getFunctionName(), null);
    ...
    private void initDots() {
        ...
        float[] dataSrc = new float[NUM_DOTS * 3]; //3D dots
        Random r = new Random();
        for (int i = 0; i < dataSrc.length; i++) {
            dataSrc[i] = r.nextFloat();
        }

        dotsMem = clCreateBuffer(context,
                CL_MEM_READ_ONLY | CL_MEM_USE_HOST_PTR,
                dataSrc.length * Sizeof.cl_float,
                Pointer.to(dataSrc), null);
    }

    void processImage(float step) {
        if (step > 6) {
            System.exit(420);
        }
        ...
        clSetKernelArg(kernel, 0, Sizeof.cl_mem, Pointer.to(outputImageMem));
        clSetKernelArg(kernel, 1, Sizeof.cl_int, Pointer.to(new int[]{SIZE}));
        clSetKernelArg(kernel, 2, Sizeof.cl_mem, Pointer.to(dotsMem));
        clSetKernelArg(kernel, 3, Sizeof.cl_uint, Pointer.to(new int[]{NUM_DOTS}));
        clSetKernelArg(kernel, 4, Sizeof.cl_float, Pointer.to(new float[]{step}));
        ...
        if (numRunTimes++ > 0) {
            totRunTimes += System.nanoTime() - start;
            if (numRunTimes % 20 == 0) {
                System.out.println(totRunTimes / numRunTimes / 1000000f);
            }
        }
    ...
    long totRunTimes = 0;
    long numRunTimes = -50; //skip in beginning
}
